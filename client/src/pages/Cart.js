        ...
        <>
          {loading ? (
            ...
            <>
              {data?.productsList.map((item) => (
                <div
                  key={item._id}
                  className="flex items-center bg-white shadow rounded-lg p-6 mb-4"
                >
                  <img
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (stripeCheckout.data?.checkout?.redirectUrl)
                window.location = stripeCheckout.data.checkout.redirectUrl;
            }}
          >
            Proceed to Checkout
          </button>
