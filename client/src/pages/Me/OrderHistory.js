        ...
        return (
          <div className='bg-white rounded shadow p-4 mt-4'>
                ...
                <div key={item.product._id} className='flex justify-between items-center mb-2'>
                  <div className='flex items-center'>
                    <Link to={`/shop/${item.product._id}`}>
                      <p className='text-lg font-semibold text-blue-400'>{item.product.name}</p>
                    </Link>
                    <p className='text-gray-600 text-sm ml-2'>x {item.quantity}</p>
                  </div>
            ...
            <div className='flex justify-between items-center'>
              ...
              <p className='text-xl font-bold'>${history.amountTotal.toFixed(2)}</p>
            </div>
          </div>
        );
      })}
